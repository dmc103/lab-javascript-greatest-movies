...
function howManyMovies(moviesArray) {
    const filtermovies = moviesArray.filter(movie => movie.dorector === 'Steven Spielberg' && movie.genre.includes (`Drama`));
...
function scoresAverage(moviesArray) {
    ...
    const filteredMovies = movies.filter((movie) => movie.score === 8);
...
function dramaMoviesScore(moviesArray) {
    ...
    const averageScore = totalScore / dramaMovies.length + 0.0;
    return averageScore;
}
